...
1. Так само я ініціалузував середовище для тестів у іншій вкладці шелу та запустив їх командою `sudo pipenv run pytest test_app.py --url http://localhost:5000` але спочатку треба перейти в папку `tests`:
    ```text
    mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5/tests (master) $ sudo pipenv run pytest test_app.py --url http://localhost:5000
    [sudo] password for mishik: 
    ============================= test session starts ==============================
    platform linux -- Python 3.9.5, pytest-6.2.5, py-1.11.0, pluggy-1.0.0
    rootdir: /home/mishik/TPIS/Mykhailo_Lakus_IK-31/lab_5/tests
    collected 4 items                                                              
    ...
    mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5/tests (master) $ 
    ```
...
Запуск сайту
```text
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ sudo make run
[sudo] password for mishik: 
Sorry, try again.
[sudo] password for mishik: 
Sorry, try again.
...
Проходження тесту:
```text
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ sudo make test-app
...
============================== 4 passed in 0.24s ===============================
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ 
```

#### 10. Зупинив проект натиснувши Ctrl+C та почистив всі ресурси `Docker` за допомогою `make`.
```text
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ sudo make docker-prune
[sudo] password for mishik: 
3e719efd3401
5d3c4e343ac1
3044d4300891
...
Запуск:
```text
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ sudo docker images
REPOSITORY          TAG        IMAGE ID       CREATED       SIZE
mishiklakus/lab4   tests      3fcb8b169ee7   2 hours ago   305MB
mishiklakus/lab4   app        8bb22e6efdd4   2 hours ago   302MB
python              3.8-slim   214d62795dbb   3 weeks ago   122MB
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ sudo make images-delete
Untagged: mishik/lab4:tests
Untagged: mishik/lab4@sha256:8967bd6199d707ece3dd7257f64dc10d884f56a60a21facfabee2745b992e451
...
Untagged: mishik/lab4:app
Untagged: mishik/lab4@sha256:c5dae1dffdc7fba23ffa264b01ec51a38dba6c9231ba79ca7b2896aeff171cab
...
services:
  hits:
    ...
    image: mishiklakus/lab4:compose-app
    container_name: app
    depends_on:
      - redis
    networks:
  ...
  tests:
    ...
    image: mishiklakus/lab4:compose-tests
...
```
```text
mishik ~/TPIS/Mykhailo_Lakus_IK-31/lab_5 (master) $ docker-compose --version
...
[sudo] password for mishik: 
REPOSITORY          TAG             IMAGE ID       CREATED          SIZE
mishiklakus/lab4   compose-tests   9db98127b13e   20 minutes ago   301MB
mishiklakus/lab4   compose-app     94499c320c52   22 minutes ago   299MB
...
services:
  app:
    ...
    image: mishiklakus/lab4:compose-jango
  ...
  monitoring:
    ...
    image: mishiklakus/lab4:compose-monitoring
